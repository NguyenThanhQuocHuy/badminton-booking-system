      ...
      <div class="untree_co-section product-section before-footer-section">
         ...
         <div class="row">  
                <?php
                    ...
                    // Lấy mã loại từ URL (?cate=1, 2,...)
                    $cate = isset($_GET['cate']) ? (int)$_GET['cate'] : 0;

                    // Lấy số trang hiện tại (?page=1, 2,...)
                    $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;

                    if ($conn && $cate > 0) {
                        // Đếm tổng số sản phẩm theo loại
                        $str = "SELECT * FROM loaisan l JOIN san s ON l.maLoai = s.maLoai WHERE l.maLoai = $cate";
                        ...
                        // Lấy sản phẩm theo loại và phân trang
                        ...
                        if (mysqli_num_rows($result) > 0) {
                            ...
                            echo "<h4>Không có sản phẩm thuộc loại này.</h4>";
                        }
                    } else {
                        echo "<h4>Không xác định được loại sản phẩm cần hiển thị.</h4>";
                    }
                ?>
