	...
	?>
      ...
      <form method="POST">
         <div class="row" style="position: relative; left: 330px; top: -40px">
            <input type="text" name="searchInput" placeholder="Nhập loại sân muốn thuê..." style="width: 800px;">
      ...
      <div class="untree_co-section product-section before-footer-section">
         ...
         <div class="row">
               ...
               $sanpham = isset($_GET['product']) ? (int)$_GET['product'] : 1;

               if ($conn) {
                  // Đếm tổng số kết quả
                  $str = "SELECT * FROM loaisan l JOIN san s ON l.maLoai = s.maLoai WHERE s.tinhTrang != 'Bảo trì'";
                  ...
                  $str = "SELECT * FROM loaisan l JOIN san s ON l.maLoai = s.maLoai WHERE s.tinhTrang != 'Bảo trì' LIMIT $page_first_result, $results_per_page";
                  $result = $conn->query($str);
                  ...
                  if (mysqli_num_rows($result) > 0) {
                     ...
                     for ($i = 1; $i <= $number_of_page; $i++) {
                        if ($i == $sanpham) {
                           echo '<a class="active" href="index.php?product=' . $i . '">' . $i . '</a> ';
                        } else {
                           echo '<a href="index.php?product=' . $i . '">' . $i . '</a> ';
                        }
                     }
                     echo "</div>";
                  }
